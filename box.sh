    ...
    blue "下载完成，正在执行 WARP 安装脚本..."
    bash "/root/menu.sh"
	}
	
	function x_ui_install(){
    wget -O "/root/install.sh" "https://raw.githubusercontent.com/vaxilu/x-ui/master/install.sh" --no-check-certificate -T 30 -t 5 -d
    chmod +x "/root/install.sh"
    blue "下载完成，正在执行 x-ui 面板管理安装脚本..."
    bash "/root/install.sh"
	}
	
	function docker_install(){
    blue "正在下载并执行 Docker 安装脚本..."
    curl -fsSL https://get.docker.com | bash -s docker
    blue "Docker 安装完成。"
	}
	
	function alist_install(){
    blue "正在下载并执行 Alist 安装脚本..."
    curl -fsSL "https://alist.nn.ci/v3.sh" | bash -s install
    blue "Alist 安装完成。"
	}
	
	function casaos_install(){
    blue "正在下载并执行 CasaOS 安装脚本..."
    bash <(curl -sSL https://linuxmirrors.cn/main.sh)
    blue "CasaOS 安装完成。"
}

	function xiaoya_install(){
    blue "正在下载并执行小雅全家桶安装脚本..."
